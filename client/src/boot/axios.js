...
export default async ({ store, Vue }) => {
    ...
    if (response.config.method === 'post') {
      if (response.status === 201) {
        if (response.data.token === undefined) { // Si no es login
          ...
          localStorage.setItem('CASHFLOW_SESSION_INFO', JSON.stringify(response.data))
        }
      }
    }
  ...
  axiosInstance.interceptors.request.use(async function (config) {
    ...
    const token = (store.state.generals.CASHFLOW_SESSION_INFO !== null) ? store.state.generals.CASHFLOW_SESSION_INFO.token : false
